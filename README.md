...
### 核心功能
- ⚡ **实时价格推送** - 通过 Bitget WebSocket 实时接收 BTC 和 ETH 最新价格
- 🪟 **迷你窗口** - 无边框、可拖拽的小窗口设计
...
- **Bitget WebSocket API** - Bitget 合约 WebSocket 实时推送价格
...
│   Go 后端核心    │  ← 业务逻辑层
├─────────────────┤
│ WebSocket客户端  │  ← 实时推送层（含代理、心跳、重连）
├─────────────────┤
│ Bitget WS API   │  ← 数据源（实时推送）
└─────────────────┘
```
...
> **注意**：`update_interval` 参数已弃用（保留用于兼容性）。WebSocket 为实时推送模式，无需设置更新间隔。

### 配置参数说明

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `proxy.enabled` | boolean | `true` | 是否启用代理（适用于WebSocket连接） |
| `proxy.address` | string | `http://127.0.0.1:7897` | 代理服务器地址 |
| `update_interval` | integer | `10` | **已弃用**（保留用于兼容性），WebSocket为实时推送 |
...
应用使用 **WebSocket 实时推送**模式，无需设置更新间隔：

- ⚡ **延迟更低**：价格变化时立即推送，延迟通常在毫秒级
- 📉 **资源占用更少**：无需定时轮询，降低服务器和客户端压力
- 🔄 **自动重连**：连接断开时自动重连，确保稳定性
- 💓 **心跳维持**：每 30 秒发送心跳，保持连接活跃

> **注**：`config.json` 中的 `update_interval` 参数已弃用，保留仅用于兼容性。
...
#### `app.go` - 业务核心
- **配置管理**：加载 `config.json`
- **代理处理**：支持多种代理配置方式（WebSocket）
- **WebSocket连接**：建立并维护与 Bitget 的 WebSocket 连接
- **实时推送**：接收并处理 Bitget 推送的实时价格数据
- **心跳机制**：定时发送心跳保持连接活跃
...
修改 `app.go` 中的 WebSocket URL 和订阅参数：
...
4. ✅ 查看控制台输出的连接状态和代理状态
5. ✅ 检查防火墙是否阻止 WebSocket 连接
6. ✅ 确认代理支持 WebSocket 协议
...
**原因**：WebSocket 连接不稳定或网络延迟

**解决方案**：
1. 检查网络连接质量
2. 查看控制台是否有频繁重连的日志
3. 确认代理服务器稳定性
4. WebSocket 为实时推送，正常延迟在毫秒级

> **注**：`update_interval` 参数已弃用，无法通过配置调整更新频率。
...
```
已加载配置文件 config.json
启动 Bitget WebSocket 连接...
✓ 使用代理连接WebSocket: http://127.0.0.1:7897
✓ 已连接到 Bitget WebSocket: wss://ws.bitget.com/v2/ws/public
✓ 已订阅 BTCUSDT 和 ETHUSDT ticker频道
✓ 订阅成功
BTC价格更新: $95234.12
ETH价格更新: $3421.45
```
...
能正常访问说明代理配置正确。WebSocket 使用相同的代理配置。
...
### v2.0.0（当前版本）
- ✅ **重大更新**：切换到 Bitget WebSocket 实时推送
- ✅ 支持 BTC 和 ETH 双币种监控
- ✅ WebSocket 代理配置支持
- ✅ 自动重连机制（指数退避）
- ✅ 心跳维持连接稳定性
...
- [Bitget API](https://bitgetlimited.github.io/apidoc/zh/mix/) - 专业的合约 WebSocket API
- [Gorilla WebSocket](https://github.com/gorilla/websocket) - 强大的 Go WebSocket 库
